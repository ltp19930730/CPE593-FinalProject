...
// construct of MandelbrotThread to initial the object with a new max iteration
MandelbrotThread::MandelbrotThread(int width,int height,int iter, QWidget *parent) : QWidget(parent){
    //init
    image = NULL;
    iterationVal = NULL;
    this->ValuesAreVaild =false;
...
double MandelbrotThread::IMAGE_VALUE_IN_SET = std::numeric_limits<double>::max();


double MandelbrotThread::zoomMultiplier = DEFAULT_ZOOM;

// insert the view parameters into a instance of a ViewParameter class
void MandelbrotThread::setViewParameters(int width,int height){
    deinit();
...
void MandelbrotThread::setMandelLocation(MandelLocation location){
    this->mandelLocation = location;
}

void MandelbrotThread::setMaxIterations(int maxIterations){
...
MandelPoint MandelbrotThread::transformViewPointToMandelPoint(ViewPoint point,ViewParmeters viewParameters,MandelLocation mandelLocation){
...
double MandelbrotThread::calculateMandelPointIterateValue(MandelPoint point){
    double x = 0;
    double y = 0;
    int i;
...
QColor MandelbrotThread::calculateImageValueColor(int value){
...
void MandelbrotThread::singleThreadTask(MandelLocation mandelLocation,ViewParmeters viewParmeters,double **imageVal,int start,int end){
...
void MandelbrotThread::mapImagValtoQimage(double **imageVal){
    for(int i=0; i<viewParameters.width; i++){
...
void MandelbrotThread::paintEvent(QPaintEvent *event){
    paintImage(this->image);
}


QString MandelbrotThread::calculateTime() {
    QTime time;
    time.start();
   if(!ValuesAreVaild){
...
void MandelbrotThread::paintImage(QImage *image){
     QTime time;
     time.start();
    if(!ValuesAreVaild){
        // multiThread to calculate
